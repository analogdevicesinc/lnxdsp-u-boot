/*
 * serial.h - common ADI uart4 serial defines SC59x.
 *
	...
	int mux_func, int port_pin)
{
	switch (port) {
	case 'A':
		writel((readl(PORTA_MUX) & ~mux_mask) | mux_func, PORTA_MUX);
		writel(port_pin, PORTA_FER_SET);
		break;
	case 'B':
	...
	case 'D':
		...
		break;
	case 'E':
		writel((readl(PORTE_MUX) & ~mux_mask) | mux_func, PORTE_MUX);
		writel(port_pin, PORTE_FER_SET);
		break;		
	}
}
