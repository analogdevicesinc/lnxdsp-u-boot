...
static int adsp_gpio_input(struct udevice *udev, unsigned pin) {
...
static int adsp_gpio_output(struct udevice *udev, unsigned pin, int value) {
	struct adsp_gpio_priv *priv = dev_get_priv(udev);
	u32 port, offset;
	void __iomem *portbase;
...
static int adsp_gpio_get_value(struct udevice *udev, unsigned pin) {
	struct adsp_gpio_priv *priv = dev_get_priv(udev);
	u32 port, offset;
	u16 val;
	void __iomem *portbase;
...
const static struct dm_gpio_ops adsp_gpio_ops = {
...
U_BOOT_DRIVER(adi_adsp_gpio) = {
	...
	.priv_auto_alloc_size = sizeof(struct adsp_gpio_priv),
