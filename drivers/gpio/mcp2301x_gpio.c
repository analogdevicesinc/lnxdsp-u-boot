...
struct mcp2301x_port {
    uint32_t io_direction_addr;
    uint32_t pullup_en_addr;
    uint32_t output_latch_addr;
    uint32_t gpio_addr;
...
static struct mcp2301x_port * mcp2301x_select_port(
      struct mcp2301x_chip *plat, uint32_t * offset)
{
    if(*offset <= PORTA_MAX){
        return &plat->port_a;
    }else if(*offset <= PORTB_MAX ){
        *offset = *offset - 8;
        return &plat->port_b;
    }else{
        pr_err("%s: Pin offset %x is greater than the " \
               "number of supported pins.\n", __func__, *offset);
    }
    return 0;
}

static int mcp2301x_direction_input(struct udevice *dev, uint32_t offset)
{
    struct mcp2301x_chip *plat = dev_get_platdata(dev);
...
static int mcp2301x_get_value(struct udevice *dev, uint32_t offset)
{
    struct mcp2301x_chip *plat = dev_get_platdata(dev);
    struct mcp2301x_port *port;
    int ret;
...
static int mcp2301x_ofdata_platdata(struct udevice *dev)
{
    ...
    if (fdtdec_get_bool(gd->fdt_blob, node, "drive-pullups")) {
...
U_BOOT_DRIVER(gpio_mcp2301x) = {
    ...
    .ofdata_to_platdata       = mcp2301x_ofdata_platdata,
    .platdata_auto_alloc_size = sizeof(struct mcp2301x_chip),
