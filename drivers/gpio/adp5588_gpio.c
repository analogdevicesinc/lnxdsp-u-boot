...
{
	struct adp5588_gpio *plat = dev_get_platdata(dev);
	unsigned bank = ADP5588_BANK(offset);
	unsigned bit = ADP5588_BIT(offset);
	int val;
...
U_BOOT_DRIVER(gpio_adp5588) = {
	...
	.ofdata_to_platdata		  = adp5588_ofdata_platdata,
	.platdata_auto_alloc_size = sizeof(struct adp5588_gpio),
