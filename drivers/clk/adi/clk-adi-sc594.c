...
static int sc594_clock_probe(struct udevice *dev) {
	...
	clk_get_by_name(dev, "dummy", &clks[ADSP_SC594_CLK_DUMMY]);
	clk_get_by_name(dev, "sys_clkin0", &clks[ADSP_SC594_CLK_SYS_CLKIN0]);
	clk_get_by_name(dev, "sys_clkin1", &clks[ADSP_SC594_CLK_SYS_CLKIN1]);
	...
	// CDU output muxes
	clks[ADSP_SC594_CLK_SHARC0_SEL] = cdu_mux("sharc0_sel", cdu + CDU_CFG0, sharc0_sels);
	clks[ADSP_SC594_CLK_SHARC1_SEL] = cdu_mux("sharc1_sel", cdu + CDU_CFG1, sharc1_sels);
	clks[ADSP_SC594_CLK_ARM_SEL] = cdu_mux("arm_sel", cdu + CDU_CFG2, arm_sels);
	clks[ADSP_SC594_CLK_CDU_DDR_SEL] = cdu_mux("cdu_ddr_sel", cdu + CDU_CFG3, cdu_ddr_sels);
